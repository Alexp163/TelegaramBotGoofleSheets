...
@bot.message_handler(commands=['start']) # декоратор, реагирующий на входящие сообщения
def start(message: telebot.types.Message):  # запуск приветствия бота и первых кнопок
    """
    Запускает работу телеграм-бота
    
    Функция, которая запускает работу бота, после ввода сообщения start и подгружает
    меню с 5-ю кнопками, а так же выводит приветствие пользователя.
    
    Args: 
    message: telebot.types.Message - аргумент для взаимодействия между пользователем и ботом.
...
def enter_cabinet(message: telebot.types.Message):
    ...
    Когда пользователь оказывается в личном кабинете запускается меню из 4-х пунктов,
...
def work_cabinet(message: telebot.types.Message):  # функция работы в личном кабинете
    """
...
def all_messages(message: telebot.types.Message):  # алгоритм работы и взаимодействия бота с пользователем
    """
    Обрабатывает ответы на запросы пользователя
    
    Функция, которая отображает взаимодействие и работу пользователя с кнопками бота.
    ...
    get_transport_company(message) - функция фиксации данных о транспортировке в гугл-таблицу.
...
def get_index_address(message: telebot.types.Message):
    ...
    Функция, которая запустит начало работы по сбору данных(индекс и адрес отделения) 
...
def entering_index_address(message: telebot.types.Message):  # функция ввода индекса и адреса
    """
    Вводит индекс и адрес отделения для доставки товара
    
    Функция, которая произведет действие по сбору данных(индекс и адрес отделения) 
    о доставке товара для пользователя.
...
def welcome_cabinet(message: telebot.types.Message):  # функция перехода кнопки в "личный кабинет"
    """
    Переходит в личный кабинет
    
    Функция, которая создаст кнопку, при нажатии которой, произойдет 
    вызов функции enter_cabinet(message) и пользователя перенаправит в личный кабинет.
    
    Args: 
    message: discord.Message - аргумент для взаимодействия между пользователем и ботом.
...
def start_pro(message: telebot.types.Message):  # функция запуска ввода данных юзера
    """
    Подготавливает к запуску ввода данных пользователя
    
    Функция запустит сбор данных о пользователе, для дальнейшего сбора данных в гугл -таблицу.
...
def get_name(message: telebot.types.Message):  # функция ввода ФИО
    """
...
def get_index(message: telebot.types.Message):  # функция ввода индекса проживания
    """
    Принимает данные о индексе проживания пользователя, с последующей обработкой.
    
    Функция выполнит сбор и обработку данных о индексе проживания пользователя,
    с добавлением полученных данных в переменную dict_customer_data.
...
def get_address(message: telebot.types.Message):  # функция записи введенных данных пользователя из бота в таблицу
    """
...
def get_transport_company(message: telebot.types.Message):  # функция фиксации данных о транспортной компании из бота в таблицу
    """
    Запускает блок кнопок о транспортной компании пользователя
    
    Функция выполнит запуск блока кнопок, для выбора пользователем транспортной компании.
